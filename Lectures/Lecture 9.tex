...
\definition{Integral closure}{
    The closure $O_K\subseteq K$ is the ring of elements in $K$ that satisfies a monic $\mathbb{Z}$-polynomial.
}
\begin{remark}
    This is a ring because the sum and products of integral elements are integral.
\end{remark}
\proposition[]{
    Every non-zero prime ideal in $O_K$ is maximal. 
}\begin{proof}
...
\end{proof}
\proposition[]{
The field of fractions of $O_K$ is $K$. 
}
\begin{proof}
...
\begin{proof}
    Let $\alpha\in K$ integral over $O_K$. Then $O_K(\alpha)$ integral over $O_K$ integral over $\mathbb{Z}$, so that $\alpha$ integral over $\mathbb{Z}$.
\end{proof}
