...
R/(p)^n \to  R/(p)^{n-1}.
\]
The inverse limit \[\ilim_n \mathbb{Z}/p^n\mathbb{Z}\defeq \mathbb{Z}_p\subseteq \prod_n \mathbb{Z}/p^n\mathbb{Z}\] with for each element $a=(...,a_2,a_1,a_0)$ we have $a_n \mapsto a_{n-1}$.
